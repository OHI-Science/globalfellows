...
<br>
<center>
![](https://docs.google.com/drawings/d/e/2PACX-1vTDLbOmBwtKrCMdtnvUWhn21Tvh3Ivqw_0Cdl6eVzyVdnaB1EQPiDu-FShlEXAqQxBNNjMQAvZCJKk8/pub?w=960&h=720)
</center>
<br>
...
Example of code to read in spatial file as a `SpatialPolygonsDataFrame`:
```
...
Example of code to read in spatial file as an `sf` and `dataframe`:
```
...
You can try it out yourself by sandwiching this code between whatever you are trying to time to see how many seconds have elapsed. Run them altogether.
...
The data is the same, it is just a different format that might be less complex and easier to process.
...
Usually, viewing the actual attribute table is really laggy in R and can cause the program to crash, especially since spatial files are often really large. But you can do it. For example, when trying to load `View(wdpa_sf)... I got a “Page Unresponsive” message: 

<br>
<center>
![](https://docs.google.com/drawings/d/e/2PACX-1vSeK-7AbPbOrdIIzUdWUcqZ2WMtuYNEEk9IEFbFAkeHuY3y6zXk_5UJLyp3ka5jkss0Kz1oYlVdVRTC/pub?w=480&h=360)
</center>
<br>

I’m just going to wait for RStudio to be responsive so I don’t have to reload all my packages and variables.

Instead of viewing the entire data table I prefer to cheat and look at a few things in the console, such as the column names. 
...
However, if you want to filter out for just a few regions, you have to do this tedious process that isn’t saved. So let’s say you want to filter another thing tomorrow using the same criteria, you may have to retype everything! And then I’ll have to open up a different window to remove the columns I don’t want to look at. 

<br>
<center>
![](https://docs.google.com/drawings/d/e/2PACX-1vQxY-4AZF_vHD6sdVsxBWZC2WFcIZSGD78oQbNiZz-b2Rj4SOcIkY8ZDyPpZ2hQvO9DXtaeAxl9TYZ5/pub?w=705&h=629)
</center>
<br>

To make this a bit more manageable to plot, I’m just going to focus on MPAs designated in year 2015 onwards. I’d still recommend not using `View()` because it’ll take forever.

Let’s say I’m just interested in status year 2015 onwards and only the no take area and name.
...
This preserves the original data and creates a new data frame with the truncated version. So now I have original dataset wdpa_sf and intermediate dataset called wdpa
...
You can plot spatial files in RStudio as well, but it’s usually a simplistic glance.
...
<br>
<center>
![](https://docs.google.com/drawings/d/e/2PACX-1vRLilUvQc8RoAa1F7CezQhQuxMVgg5-4xLZszdYptcPN6ZtkP1_mAp7ebBGHAyJ0xGqHXE2SJROD-1i/pub?w=936&h=472)
</center>
<br>

What are all the values? So the [1] specifies that you are taking the values in the first column, which is meaningless as it is just a WDPA id number. Let’s say I don’t want any special values:
...
<br>
<center>
![](https://docs.google.com/drawings/d/e/2PACX-1vQU7mZtHhqu3FZzxp1aRAJhTv9YSUTlLfLACNSuCXYCziJLiWYr7KLOBSYusjGt9ZEuDME9sDTtmaxH/pub?w=929&h=447)
</center>
<br>

This will just plot the geometry or shape.

You can always create annotations to take a better look at the values, but this would require an understanding of the columns or attributes first. It looks like I can categorize the values by No Take Area, Status Year (or year it was designated) or perhaps IUCN category.
...
plot(wdpa_sf[“no_tk_area”], axes = TRUE)
```

<br>
<center>
![](https://docs.google.com/drawings/d/e/2PACX-1vTqOSaFCXoAplLd2pD-2Vwb6g_FpPeXEOaQT3lgj2Xbc62cL-FJD9miXtjdfb7bfEimqGXHC1bI2155/pub?w=705&h=575)
</center>
<br>

The axes annotation brought out the latitude and longitude coordinate axes.
